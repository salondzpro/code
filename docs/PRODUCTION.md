...
19bis. **Espace d'administration de la place de marché** — analyse et conception le 20 sept. dans `docs/ADMIN.md`, **lot 1 (voir) livré le même jour**. Constat de départ : il n'existait AUCUN administrateur (le rôle vaut `client` ou `pro`), aucune suspension de salon ou de client par la plateforme, aucune modération des avis, aucun signalement, aucun journal — dépublier un salon abusif demandait d'ouvrir la base à la main. Livré : migration 0044 (`platform_admins`, `admin_audit`, quatre fonctions de lecture, RLS sans politique et droits révoqués), garde `requireAdmin` (quatrième maillon après `requireAuth` → `requireProfile`, refus journalisés), routes `/v1/admin/*` (120 req/min, `private, no-store`), et les écrans sous `/admin` : vue d'ensemble (aujourd'hui, 30 jours, place de marché, santé du cron), professionnels liste + fiche (ce que le pro voit lui-même : catalogue, équipe, horaires, avis), comptes liste + fiche (historique, avis, salons qui l'ont bloqué), recherche de rendez-vous pour les litiges, journal. La consultation d'une fiche nominative laisse une trace ; une liste, non. Aucun lien ne mène à `/admin` : l'adresse se tape et le garde serveur est la seule barrière (un 403 renvoie à l'accueil sans message). Accès donné et retiré par `scripts/admin.mjs` ; `revoke` désactive, ne supprime jamais. Test e2e 37/37. Compte `owner` : `admin@salondz.com` (20 sept.). **Lot 2 livré le 21 sept.** (migration 0045) : suspendre un salon à deux degrés (gelé / masqué), suspendre un client sur toute la place de marché, masquer un avis, annuler au nom de la plateforme, et piloter l'espace d'un professionnel (**jeton de contrôle** signé, valable 2 h, lié à l'administrateur et au salon, envoyé dans `X-Admin-Control` ; entrée, sortie et chaque écriture journalisées, bandeau permanent). Motif obligatoire partout, montré à la personne concernée. **Déployé et vérifié en production le 21 sept.** (`pnpm check:control`, 30/30) — migration 0045 appliquée le même jour, après sauvegarde. **Reste** : les signalements par les utilisateurs, et le lot 3 (réglages, contenu, export du journal, second facteur).
