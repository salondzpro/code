...
19bis. **Espace d'administration de la place de marché** — analyse et conception faites le 20 sept. dans `docs/ADMIN.md` (aucun code écrit). Constat : il n'existe AUCUN administrateur (le rôle vaut `client` ou `pro`), aucune suspension de salon ou de client par la plateforme, aucune modération des avis, aucun signalement, aucun journal. L'API contournant déjà la RLS avec la clé secrète, un quatrième garde `requireAdmin` s'insère sans refonte. Trois lots proposés, le premier en lecture seule. **Sept décisions attendues du propriétaire** (section 9 du document) avant d'écrire une ligne.
