...
- **Web → Render, site statique** (gratuit, CDN, hors quota des 750 h) : https://salondz.com (alias https://salondz.onrender.com), déclaré dans `render.yaml` (build `pnpm --filter @salondz/web build`, sortie `apps/web/dist`, règle SPA `/* → /index.html`, en-têtes cache/sécurité, variables `VITE_*`). Création/mise à jour sans dashboard : `node --env-file=.env scripts/render-web.mjs` (clé `RENDER_API_KEY` dans `.env`). `_redirects`/`_headers` restent pour Cloudflare Pages si l'on y migre un jour.
