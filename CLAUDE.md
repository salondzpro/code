...
- `pnpm --filter @salondz/web test:e2e` : parcours complet pro + client dans Chromium headless (playwright-core) ; exige `pnpm dev:api` (8080) + `pnpm dev:web` (9000) lancés, `PLAYWRIGHT_CHROME` si Chrome n'est pas installé. Captures dans `apps/web/test/shots/`.
- `pnpm test:e2e:mobile` : parcours client + pro sur l'app mobile rendue par Expo web (`apps/mobile/test/smoke.e2e.mjs`) ; exige `pnpm dev:api` et, depuis `apps/mobile`, `CI=1 EXPO_PUBLIC_API_URL=http://localhost:8080 npx expo start --web --port 8082`. Captures dans `apps/mobile/test/shots/`.
...
- API locale : `pnpm dev:api` sur le port `PORT` du `.env` (8080 en local comme sur Fly/Koyeb) ; web toujours sur http://localhost:9000 (Vite `strictPort`).
