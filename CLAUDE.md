...
- **Deux échelles ont été MESURÉES sur la référence du marché (Planity, navigateur en conditions téléphone) et l'emportent sur le fichier de design** : la typographie (racine 14 px ; six tailles 12 / 14 / 16 / 20 / 24 / 32 ; poids ≤ 600 au-dessus de 20 px) et les arrondis (**12 px** pour les cartes, **8 px** pour tout le petit mobilier — boutons, champs, créneaux, vignettes —, 16 px pour les feuilles, pilules et cercles inchangés). Relevé : 8 px sur 99 éléments, 12 px sur 21, pilules sur 36. Ne pas « rétablir » 20 / 16 / 14 px : c'est un air de jouet, pas un outil de travail. Tout passe par les jetons (`--radius-*`, `R`), jamais par une valeur en dur.
