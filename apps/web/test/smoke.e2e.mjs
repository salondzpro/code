...
try {
  ...
  await step('client: coordonnées → récapitulatif → confirmation', async () => {
    ...
    // La ligne du prix, désignée par son sous-titre : unique dans la page et sans ambiguïté,
    // là où « 800 DA » apparaît à plusieurs endroits dont certains masqués.
    await c.getByText('Paiement sur place, aucun acompte').waitFor();
  ...
  await step('pro: fiche client (statistiques, bloquer puis débloquer)', async () => {
    ...
    await sheet.getByText(/Prochain rendez-vous|Aucun rendez-vous prévu/).first().waitFor();
