...
export function Step1Market() {
  ...
  return (
    <Screen bottom={SHEET_PAD} gap={16}>
      ...
      {CARDS.map((c) => (
        <button key={c.id} type="button" onClick={() => setMarket(c.id)} aria-pressed={market === c.id} className={`relative h-[10rem] w-full overflow-hidden rounded-[0.857rem] text-left ${market === c.id ? 'ring-2 ring-ink' : ''}`}>
