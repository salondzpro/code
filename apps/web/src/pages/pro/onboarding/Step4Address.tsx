...
import { queryKeys, useProSalonMutations } from '@salondz/api-client';
import { WILAYAS, categoriesForMarket, wilayaName } from '@salondz/constants';
...
export function Step4Address() {
  ...
  const [wilaya, setWilaya] = useState(draft.wilayaCode ?? 16);
  const [zone, setZone] = useState(draft.zone ?? '');
  const [home, setHome] = useState(draft.homeService ?? false);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);

  if (!draft.market || !draft.name) return <Navigate to={stepPath(1)} replace />;
  ...
  const submit = async (e?: FormEvent) => {
    ...
    try {
      ...
      const [coverUrl, logoUrl] = await Promise.all([files.cover ? uploadSalonPhoto(salon.id, files.cover) : null, files.logo ? uploadSalonPhoto(salon.id, files.logo) : null]);
  ...
  return (
    <Screen bottom={SHEET_PAD} gap={16}>
      <StepBar step={4} backTo={stepPath(3)} />
      ...
      <StepSheet onClick={() => void submit()} busy={busy} />
    </Screen>
  );
}
