  ...
  return (
    <Screen bottom={NAV_PAD} gap={16}>
      <TopBar
        backTo="/pro/clients"
        right={
          c.blocked ? <Badge tone="cn">Client bloqué</Badge> : <Badge tone="ok">Client actif</Badge>
        }
      />
      ...
      <div className="g2">
        <Button
          onClick={() =>
            navigate(
              `/pro/rendez-vous/nouveau?name=${encodeURIComponent(c.name)}${c.phone ? `&phone=${encodeURIComponent(c.phone)}` : ''}`,
            )
          }
          disabled={c.blocked}
        >
