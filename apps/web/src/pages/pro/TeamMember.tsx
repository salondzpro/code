...
import { Avatar, BottomSheet, Button, I, Segmented, Skeleton, Toggle, TopBar } from '@/components/ui';
...
export function TeamMember() {
  ...
  const hoursSummary = hours.isPending ? '…' : hours.data && hours.data.length > 0 ? `Personnalisés · ${[...new Set(hours.data.map((h) => h.dayOfWeek))].length} jour${new Set(hours.data.map((h) => h.dayOfWeek)).size > 1 ? 's' : ''}` : 'Horaires du salon';
    ...
    <Screen bottom={NAV_PAD} gap={16}>
      ...
      <div className="flex items-center gap-3.5">
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="p block">{isOwner ? 'Propriétaire' : member.isActive ? 'Membre actif' : 'Inactif — masqué à la réservation'}</span>
        </span>
      ...
      <div className="crd !gap-0 !py-1">
        <button type="button" className="li w-full !py-4 text-left" onClick={() => navigate(`/pro/equipe/${member.id}/prestations`)}>
        ...
        <button type="button" className="li w-full !py-4 text-left" onClick={() => navigate(`/pro/equipe/${member.id}/horaires`)}>
          <span className="flex items-center gap-3.5">
            <span className="flex h-10 w-10 items-center justify-center rounded-full bg-fill">
          ...
          {[0, 1, 2, 3, 4, 5, 6].map((d) => {
            ...
            return (
              <span key={d} className="flex justify-between text-[0.875rem]">
                <span className={ranges.length ? '' : 'text-subtle'}>{DAY_LABELS_SHORT_FR[d as 0]}</span>
      ...
      {!isOwner &&
        (confirmRemove ? (
          ...
          <button type="button" className="py-2 text-[0.875rem] text-danger" onClick={() => setConfirmRemove(true)}>
            Retirer de l'équipe
          </button>
        ))}
    </Screen>
...
export function TeamMemberServices() {
  ...
  const [error, setError] = useState<string | null>(null);
  if (!salon) return <Splash />;
  if (!member) return <Screen bottom={NAV_PAD}><TopBar backTo="/pro/equipe" /><p className="p">Membre introuvable.</p></Screen>;
  ...
  return (
    <Screen bottom={SHEET_PAD} gap={16}>
      ...
      <p className="p">Le membre n'est proposé aux clients que pour les prestations qu'il réalise.</p>
...
export function TeamMemberHours() {
  ...
  return (
    <Screen bottom={SHEET_PAD} gap={16}>
      ...
      />
      {hours.isPending || rows.length === 0 ? <Skeleton className="h-[7.5rem]" /> : custom ? <WeekHoursEditor rows={rows} onChange={setRows} closedLabel="Repos" /> : <p className="p">Ce membre est réservable sur tous les horaires d'ouverture du salon.</p>}
