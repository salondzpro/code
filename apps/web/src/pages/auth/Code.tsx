  ...
  return (
    <Screen className="min-h-dvh" gap={16}>
      ...
      {status === 'idle' && attempts === 0 && (
        <div className="rounded-[1.25rem] bg-ok-bg p-4 text-[1rem] leading-[1.45]">
          <div className="mb-3 flex items-center gap-3">
            ...
            <span>
              <span className="block text-[0.857rem] font-semibold">{isEmail ? 'E-mail · Salon DZ' : flow.channel === 'sms' ? 'SMS · Salon DZ' : 'WhatsApp · Salon DZ'}</span>
      ...
      <div className="flex gap-2.5" aria-label="Code à 4 chiffres">
        {digits.map((d, i) => (
          <input
            ...
            className={`inp !p-0 h-[4.25rem] text-center text-[1.143rem] font-medium${d ? ' f' : ''}${status === 'wrong' ? ' err' : ''}`}
            inputMode="numeric"
      ...
      {status === 'idle' && attempts === 0 && <p className="p text-[1rem]">Coller automatiquement depuis {isEmail ? 'votre messagerie' : 'WhatsApp'}</p>}
      ...
      {!expired && status !== 'network' && (
        <Card className="!flex-row items-center justify-between">
          <span>
            <span className="block text-[0.857rem]">Rester connecté</span>
            <span className="p block text-[1rem]">Session illimitée · aucun code à la prochaine visite</span>
          </span>
      ...
      {status === 'network' ? (
        ...
        <>
          ...
          {status === 'wrong' && (
            <button type="button" className="text-center text-[1rem] text-muted underline" onClick={() => void resend()} disabled={resendIn > 0}>
