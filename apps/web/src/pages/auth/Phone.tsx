      ...
      <form onSubmit={submit} className="flex flex-col gap-4">
        {useEmail ? (
          ...
          <div className="flex gap-2.5">
            <div className="flex flex-none items-center gap-2 rounded-[0.875rem] bg-fill px-4 text-[0.857rem] font-medium" aria-label="Indicatif +213">
              +213
        ...
        {error ? (
          ...
          <p className="p text-[1rem]">{useEmail ? 'Le code arrivera par e-mail.' : 'Format algérien · +213 XX XX XX XX'}</p>
        )}
        <Button type="submit">Recevoir le code</Button>
        {EMAIL_FALLBACK && (
          <button type="button" className="text-center text-[1rem] text-muted underline" onClick={() => setUseEmail((v) => !v)}>
