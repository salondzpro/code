...
export function Bookings() {
  ...
  const [scope, setScope] = useState<'upcoming' | 'past' | 'cancelled'>(params.get('scope') === 'past' ? 'past' : params.get('scope') === 'cancelled' ? 'cancelled' : 'upcoming');
    ...
    <Screen bottom={NAV_PAD} gap={16}>
      ...
      {list.isPending ? (
        ...
        <div className="flex flex-col items-center gap-3 px-4 pt-14 text-center">
          <div className="text-[1.125rem] font-bold">{scope === 'upcoming' ? 'Aucun rendez-vous à venir' : scope === 'cancelled' ? 'Aucun rendez-vous annulé' : 'Aucun rendez-vous passé'}</div>
