...
export function DayScroller({ selected, onSelect, disabledDays }: { selected: string; onSelect: (dateKey: string) => void; disabledDays?: readonly number[] }) {
  ...
  useEffect(() => {
    ...
    ref.current?.scrollTo({ x: Math.max(0, idx * (CELL + GAP) - width.current / 2 + CELL / 2), animated: true });
  }, [selected, days]);
  ...
  return (
    <ScrollView
      ...
      onLayout={(e) => {
        ...
        const idx = days.indexOf(selected);
        if (idx >= 0) ref.current?.scrollTo({ x: Math.max(0, idx * (CELL + GAP) - width.current / 2 + CELL / 2), animated: false });
      }}
      ...
      {days.map((d) => {
        ...
        return (
          <Pressable key={d} accessibilityRole="radio" accessibilityState={{ selected: on }} accessibilityLabel={`${DAY_LABELS_SHORT_FR[dow]} ${dayNumber(d)}`} onPress={() => onSelect(d)} style={{ width: CELL, alignItems: 'center', gap: 3, paddingVertical: 7, borderRadius: 12, backgroundColor: on ? C.ink : 'transparent' }}>
            ...
            <Tx size={13} weight={600} lh={16} color={on ? '#fff' : out ? C.disabled : C.text} style={d === today && !on ? { textDecorationLine: 'underline' } : undefined}>
              {dayNumber(d)}
            </Tx>
          </Pressable>
        );
