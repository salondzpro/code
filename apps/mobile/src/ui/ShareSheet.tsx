      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
        {items.map((it) => (
          <Pressable key={it.label} accessibilityRole="button" accessibilityLabel={it.label} onPress={it.onPress} style={{ width: '22%', flexGrow: 1, alignItems: 'center', gap: 6 }}>
            <View style={{ width: 62, height: 62, borderRadius: R.card, borderWidth: 1, borderColor: C.line, backgroundColor: C.surface, alignItems: 'center', justifyContent: 'center' }}>{it.icon}</View>
