...
export default function Profile() {
  ...
  return (
    <Screen gap={13} bottom={NAV_PAD}>
      ...
      <Grid cols={3}>
        {[
          ...
          <Card key={x.l} gap={3} pad={12} style={{ paddingVertical: 16 }}>
            ...
            <Tx size={11.5} color={C.muted} lh={15.5} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.85}>
              {x.l}
            </Tx>
          </Card>
        ))}
      ...
      <Pressable accessibilityRole="link" onPress={() => router.push(salon ? '/(pro)' : '/pro-bienvenue')} style={{ flexDirection: 'row', alignItems: 'center', gap: 13, backgroundColor: C.fill, borderRadius: R.cardSm, padding: 13 }}>
        <View style={{ width: 55, height: 55, borderRadius: 28, borderWidth: 1, borderColor: C.line, backgroundColor: C.surface, alignItems: 'center', justifyContent: 'center' }}>
