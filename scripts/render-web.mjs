...
if (apiService) {
  ...
  if (!origins.has(siteUrl)) {
    ...
    console.log(`CORS_ORIGINS de l'API mis à jour : ${[...origins].join(',')} (redéploiement automatique de l'API).`);
  } else {
...
if (deployId) {
  process.stdout.write('Déploiement ');
  for (let i = 0; i < 90; i++) {
    const d = await get(`/services/${site.id}/deploys/${deployId}`);
    process.stdout.write(`${d.status} `);
    if (d.status === 'live') break;
    if (/failed|canceled|deactivated/.test(d.status)) fail(`\nDéploiement ${d.status} — voir les logs : https://dashboard.render.com/static/${site.id}`);
