...
export function makeQueries(api: ApiClient) {
  return {
    ...
    meStats: () => queryOptions({ queryKey: queryKeys.meStats, queryFn: () => api.me.stats(), staleTime: 30_000 }),
    ...
    pro: {
      ...
      clients: () => queryOptions({ queryKey: queryKeys.pro.clients, queryFn: () => api.pro.clients.list({ limit: 100 }), staleTime: MIN }),
      client: (key: string) => queryOptions({ queryKey: queryKeys.pro.client(key), queryFn: () => api.pro.clients.one(key), staleTime: MIN, enabled: !!key }),
